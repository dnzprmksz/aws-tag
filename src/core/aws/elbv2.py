...
class ELBv2(BaseAwsService):
    ...
    def _list_resources(self, filters: List[Filter]) -> List[Resource]:
        ...
        while 'NextMarker' in response:
            response = self.client.describe_load_balancers(PageSize=limit, Marker=response['NextMarker'])
            resources += self.__list_response_to_resources(response)
    ...
    def __list_response_to_resources(response) -> List[Resource]:
        ...
        """
        resources = [
            Resource(name=item['LoadBalancerName'], arn=item['LoadBalancerArn'])
    ...
    def _get_resource_tags(self, resource: Resource) -> List[Tag]:
        ...
        """
        response = self.client.describe_tags(ResourceArns=[resource.arn])
